...
void JobProcessTriangleInteractionRequests::update(WECS* ecs, HEntity hEntity, CTriangle& triangle,  CInteractionRequests& interactionRequests)
{
	for (int m = 0; m < interactionRequests.size(); m++)
	{
		HEntity hRequest = interactionRequests[m];
		CInteractionRequest& interactionRequest = ecs->getComponent<CInteractionRequest>(hRequest);
		const DRayf& ray = interactionRequest.ray;
		...
		ecs->addComponent<CInteractionTriangle>(hRequest, std::move(interactionTriangle));
	}
...
void JobProcessTriangleFullInteractionRequests::update(WECS* ecs, HEntity hEntity, CTriangle& triangle,
													   CInteractionFullRequests& interactions)
{
	for (uint32_t m = 0; m < interactions.size(); m++)
	{
		HEntity hRequest = interactions[m];
		const CInteractionRequest& interactionRequest = ecs->getComponent<CInteractionRequest>(hRequest);
		CInteractionTriangle& interactionTriangle = ecs->getComponent<CInteractionTriangle>(hRequest);
		const CTriangleMesh& triangleMesh = ecs->getComponent<CTriangleMesh>(triangle.hMesh);
