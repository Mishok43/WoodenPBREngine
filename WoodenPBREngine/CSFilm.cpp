...
void JobGenerateFilmTiles::update(WECS* ecs, HEntity hEntity, CFilm& film)
{
	...
	for (uint32_t i = 0; i < maxConcurrentProcessTiles; i++)
	{
		...
		CFilmTile tile;
		tile.nSamples1DPerCameraSample = 30;
		tile.nSamples2DPerCameraSample = 30;
...
void JobAccumalateLIFromSamples::update(WECS* ecs, uint8_t iThread) 
{
	...
	for_each([&](HEntity, CFilmTile& tile)
	{
		...
		{
			const static MitchellFilter filter(4.0, 1.0/3.0f, 1.0/3.0f);
			...
			pMax = minv(pMax, tile.pixelBounds.pMax - DPoint2i(1, 1));
...
{
	ecs->deleteEntitiesOfComponents<CCameraSample>();
...
void JobOutputFilmTitles::update(WECS* ecs, uint8_t iThread)
{
	...
	for_each([&](HEntity, CFilmTile& tile)
	{
		...
		{
			if (tile.pixFilterWeightAccumulated[i] == 0.0f)
			{
			...
			DVector3f rgb = toRGB(tile.pixLiAccumulated[i] / 32.0) /(tile.pixFilterWeightAccumulated[i]* 5000000.f);
			rgb = wml::clamp(rgb, 0.0, 1.0f)*255.0f;
