...
class STexture2DSampler
{
public:

	template<typename T>
	static auto triangle(const DPoint2f& st,
						 ...
						 uint32_t mip)
	{
		...
		s0n = max(s0n, 0);
		t0n = max(t0n, 0);
		return	tex.texel(s0, t0, mip).unpack()*(1 - ds) * (1 - dt) +
			tex.texel(s0, t0n, mip).unpack()*(1 - ds) * dt +
			tex.texel(s0n, t0, mip).unpack()*ds * (1 - dt) +
			tex.texel(s0n, t0n, mip).unpack()*ds * dt;
	}

	template<typename T>
	static auto point(const DPoint2f& st,
					  const Texture2DBase<T>& tex,
					  uint32_t mip)
	{
		uint32_t nLevels = tex.mips.size();
		...
		s0 = max(s0, 0);
		t0 = max(t0, 0);
		return tex.texel(s0, t0, mip).unpack();
	}
};

class STexture3DSampler
{
public:	
	template<typename T>
	static auto point(const DPoint3f& st,
				   const Texture3DBase<T>& tex,
				   uint32_t mip)
	{
		uint32_t nLevels = tex.mips.size();
		mip = wml::clamp(mip, 0, nLevels - 1);
		float s = st[0] * tex.resolutions[mip].x() - 0.5f;
		float t = st[1] * tex.resolutions[mip].y() - 0.5f;
		float m = st[2] * tex.resolutions[mip].z() - 0.5f;
		int s0 = std::floor(s), t0 = std::floor(t), m0 = std::floor(m);
		s0 = max(s0, 0);
		t0 = max(t0, 0);
		m0 = max(m0, 0);
		return tex.texel(s0, t0, m0, mip).unpack();
	}
};


class STexture3DSamplerPoint
{
...
public:
	...
	static auto eval(
		...
		const Texture2DBase<T>& tex)
	{
		return STexture3DSampler::point(st.p, tex, 0);
	}
};

class STexture2DSamplerAnisotropic
{
public:
	template<typename T>
	static auto eval(
		...
		const Texture2DBase<T>& tex)
	{
		...
		if (minorLength == 0)
		{
			return STexture2DSampler::triangle(st.p, tex, 0);
		}
	...
	template<typename T>
	static auto ewa(const CTextureMappedPoint& p,
					const Texture2DBase<T>& tex,
					...
					uint32_t mip)
	{
		...
		uvSqrt *= 2 * invDet;
		//st = DVector2f(-0.46f);
		//uvSqrt = DVector2f(2.0f);
		DVector2f st0 = clamp(st - uvSqrt, 0.0f, (DVector2f)tex.resolutions[mip]-1.0f);
		DVector2f st1 = clamp(st + uvSqrt, 0.0f, (DVector2f)tex.resolutions[mip]-1.0f);
		...
		for (int it = t0; it <= t1; ++it)
		{
			...
			for (int is = s0; is <= s1; ++is)
			{
				...
				if (r2 < 1)
				{
					...
					sum += tex.texel(is, it, mip).unpack() * weight;
					sumWts += weight;
				}
...
class STexture2DSamplerIsotropic
{
...
public:
		   ...
		   const Texture2DBase<T>& tex) 
	{
		...
		{
			return tex.texel(0, 0, nLevels - 1).unpack();
		}
		else
		{
			int iLevel = std::floor(level);
			float delta = level - iLevel;
			return lerp(STexture2DSampler::triangle(st.p, tex, iLevel), STexture2DSampler::triangle(st.p, tex, iLevel + 1), delta);
		}
	}
};

class STexture2DSamplerPoint
{
...
public:
	...
	static auto eval(
		...
		const Texture2DBase<T>& tex)
	{
		return STexture2DSampler::point(st.p, tex, 0);
	}
};
...
public:
	...
	static void load(std::map<std::string, HEntity>& textures, CTextureBindingBase<CompT>& textureInfo)
	{
		...
		std::vector<T, AllocatorAligned2<T>> texels = readImagePNG<T>(textureInfo.filename, resolutions);
	...
	static CompT generateMIPS(std::vector<T, AllocatorAligned2<T>> texels, DPoint2i resolution)
	{
		...
		for (uint16_t i = 1; i < nLevels; i++)
		{
			...
			for (uint32_t y = 0; y < res.y() - 1; y++)
			{
				...
				{
					tex.texel(x, y, i) = (tex.texel(2 * x, 2 * y, i - 1).unpack() + tex.texel(2 * x + 1, 2 * y, i - 1).unpack() +
										  tex.texel(2 * x, 2 * y + 1, i - 1).unpack() + tex.texel(2 * x + 1, 2 * y + 1, i - 1).unpack())*0.25;
				}

				tex.texel(res.x() - 1, y, i) = (tex.texel(tex.resolutions[i - 1].x() - 1, 2 * y, i - 1).unpack() +
												tex.texel(tex.resolutions[i - 1].x() - 1, 2 * y + 1, i - 1).unpack())*0.5;
			}

			for (uint32_t x = 0; x < res.x() - 1; x++)
			{
				tex.texel(x, res.y() - 1, i) = (tex.texel(2 * x, tex.resolutions[i - 1].y() - 1, i - 1).unpack() +
												tex.texel(2 * x + 1, tex.resolutions[i - 1].y() - 1, i - 1).unpack())*0.5;
			}
	...
	static void load3D(std::map<std::string, HEntity>& textures, CTextureBindingBase<CompT>& textureInfo)
	{
		...
		HEntity hTexture = ecs.createEntity();
		CompT texRGB = generateMIPS3D<T, CompT>(std::move(texels), resolutionReal);
		ecs.addComponent<CompT>(hTexture, std::move(texRGB));

		textureInfo.tex = hTexture;
		textureInfo.isLoaded = true;
		textureInfo.resX = resolutionReal.x();
		textureInfo.resY = resolutionReal.y();
		textures[textureInfo.filename] = hTexture;
	}
	...
	static std::vector<T, AllocatorAligned2<T>> readImagePNG(const std::string &name, DPoint2i& resolution)
	{
		...
		{
			for (unsigned int x = 0; x < w; ++x, src += tmp.size())
			{
				if constexpr (tmp.size() == 1)
				{
					tmp[0] = src[0]/255.0f;
				}
				else
				{
					auto tmpUnpacked = tmp.unpack();
